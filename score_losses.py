...
class VPSDELoss:
    ...
    def get_loss(self, model, x, y, context=None):
        y = dropout_label_for_cfg_training(
            ...
            self.n_classes,
            self.label_unconditioning_prob,
            x.device,
        )

        t = (1.0 - self.eps_t) * torch.rand(
...
class VLoss:
    ...
    def _t_given_logsnr(self, logsnr):
        return 2.0 * np.arccos(1.0 / np.sqrt(1.0 + np.exp(-logsnr))) / np.pi
    ...
    def get_loss(self, model, x, y, context=None):
        ...
        t = (self.eps_max - self.eps_min) * torch.rand(
            (x.shape[0], self.n_noise_samples), device=x.device
        ) + self.eps_min
        sigma = self._sigma(t)
...
class EDMLoss:
    ...
    def get_loss(self, model, x, y, context=None):
        ...
        pred = model(
            ...
            y,
            context=context_repeated.reshape(-1, *context.shape[1:]),
        ).reshape(x.shape[0], self.n_noise_samples, *x.shape[1:])
        loss = w * (pred - x_repeated) ** 2.0
        loss = torch.mean(loss.reshape(loss.shape[0], -1), dim=-1)
        return loss
